...
GPIO_Handler_t userLedRed = { 0 }; // PB15
GPIO_Handler_t userLedGreen = { 0 }; // PB14
GPIO_Handler_t userLedBlue = { 0 }; // PB13
...
void initSys(void) {
	...
	userLedRed.pinConfig.GPIO_PinNumber = PIN_6;
	...
	/* Configuramos el PWM */
	rgbTimer.pTIMx = TIM4;
	rgbTimer.TIMx_Config.TIMx_Prescaler = 1600;
	rgbTimer.TIMx_Config.TIMx_mode = TIMER_UP_COUNTER;
	rgbTimer.TIMx_Config.TIMx_InterruptEnable = TIMER_INT_ENABLE;
	timer_Config(&rgbTimer);
	timer_SetState(&rgbTimer, TIMER_ON);

	rgb.ptrTIMx = TIM4;
	rgb.config.channel = PWM_CHANNEL_1;
...
void changePwm(PWM_Handler_t *ptrTIMx, uint8_t channel) {
	switch (channel) {
	case 0: {
		ptrTIMx->config.channel = PWM_CHANNEL_1;
		break;
	}
	case 1: {
