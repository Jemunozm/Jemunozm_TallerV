...
int main(void)
{
	 ...
	 * Además que aunque tuvieramos elvalor del PIN el registro IDR, podría
	...
	while(1){
		...
		conteo = 0;
		uint8_t posPin = 0;
		for (uint8_t i=0;	(i< 60 && gpio_ReadPin(&userBoton) == 1) ;	++i){
		...
		gpio_WritePin(&userLed, RESET);
		conteo = 60;
		for (uint8_t i=0;	(i< 60 && gpio_ReadPin(&userBoton) == 0);	++i){
